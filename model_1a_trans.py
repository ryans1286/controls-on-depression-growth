...
"""
parameter_file = 'input_parameters_1a_trans-2.csv'

"""
Create a data output directory
"""
output_dir = 'output-1a-transects/'
...
for sim in range(number_of_simulations):
    ...
    for ts in range(t_steps):
        ...
        #Save transects and pickle the model each simulation year
        if ts*dt % 1 == 0:
            #Create a transect
            time = int(round(ts * dt))

            z_transects[time] = mg.node_vector_to_raster(z)[nr//2, :]
            s_transects[time] = copy.deepcopy(mg.node_vector_to_raster(s)[nr//2, :])
            
            #Pickle the model
            pickle_fname1 = output_dir + "/" + 'sim_' + str(sim_num) + "_s_time={}.pkl".format(time)
            pickle_fname2 = output_dir + "/" + 'sim_' + str(sim_num) + "_z_time={}.pkl".format(time)
            pf1 = open(pickle_fname1, 'wb')
            pf2 = open(pickle_fname2, 'wb')
            pickle.dump(s, pf1)
            pickle.dump(z, pf2)
            pf1.close()
            pf2.close()
    ...
    #Save topographic transects csv    
    z_trans_fname = os.path.join(output_dir, 'sim_' + str(sim_num)  + "_z_trans.csv")
    ...
    s_trans_fname = os.path.join(output_dir,  'sim_' + str(sim_num) + "_s_trans.csv")
